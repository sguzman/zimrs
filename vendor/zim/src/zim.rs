...
fn parse_article_list(master_view: &Mmap, ptr_pos: u64, count: u32) -> Result<Vec<u32>> {
    // Some v6 archives set this pointer to 0xFFFF_FFFF_FFFF_FFFF when the
    // title pointer table is absent. In that case, fall back to an identity
    // mapping into the URL list.
    if ptr_pos == u64::MAX {
